...
export default function MovieDetails() {
  ...
  const onAddMovieToFavourites = e => {
    ...
    addMovieToFavourites(movieId, currentUser).then(res => {
  ...
  const onRemoveFromFavourites = e => {
    ...
    removeMovieFromFavourites(movieId, currentUser).then(res => {
      console.log(res);
    ...
    <>
      <div className="details_container_ma">
            ...
            [
              currentUser === currentMovie.creator ? (
                <div className="creator_buttons">
                  <Link
                    to={`/edit/${currentMovie.movieId}`}
                    className="personal_btn"
                  >
                    Edit
                  </Link>
                  <Link to="#" onClick={onMovieDelete} className="personal_btn">
                    Delete
                  </Link>
                </div>
              ) : (
                [
                  ifAdded ? (
                    ...
                    <div className="creator_buttons">
                      <Link
                        to="#"
                        onClick={onAddMovieToFavourites}
                        className="personal_btn_guest"
                      >
                        Add to favourites
                      </Link>
        ...
        <div className="movie_information">
          <h1>
            Movie Title: <span>{currentMovie.name}</span>{" "}
          </h1>
          <h2>
            Overview: <span>{currentMovie.overview}</span>
          </h2>
          <h2>
            Genre: <span>{currentMovie.genre}</span>
          </h2>
          <h2>
            Rating: <span>{currentMovie.rating}/10</span>
          </h2>
          <h2>
            Runtime: <span>{currentMovie.runtime}</span>
          </h2>
          <h2>
            Budget: <span>${currentMovie.budget}</span>
          </h2>
        </div>
      </div>
